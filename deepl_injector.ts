...
(() => {
  ...
  window.addEventListener("message", (msg) => {
    if (
      !msg.data.type || msg.data.type !== "zhmushan_translator_deepl_request"
    ) return;
    ...
    const intervalId = setInterval(() => {
      ...
      if (result.length > 0) {
        ...
        window.parent.postMessage(
          { type: "zhmushan_translator_deepl_response", status: 200, result },
        ...
        window.parent.postMessage(
          {
            type: "zhmushan_translator_deepl_response",
            status: 504,
